public class TreatmentQueue {
    ...
    public TreatmentRequest dequeuePriority() {  // dequeue method for priority queue.
        if (emergency.head != null) {
            return emergency.dequeue(); // If it is emergency firstly, delete from emergency queue.
        } else if (normal.head != null) {   // If it is normal, delete from normal queue.
            return normal.dequeue();
        } else {
            throw new RuntimeException("Empty queue");  // If the qeuseue is empty print "Empty queue"
        }
    }
}
